...
class StaffMessageEchoPacketListener : EchoPacketListener {
    ...
    fun on(
        ...
        val message = ComponentBuilder().append("§d§l[S] ").append("§7(${bukkitApplication?.server?.getFancyDisplayName() ?: bukkitApplication?.getFancyDisplayName()}) ").append("${group.getColoredPrefix()}${user.name}: ").append("§f${packet.message}").create()
